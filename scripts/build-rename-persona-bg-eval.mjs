/**
 * Builds scripts/.generated-rename-persona-bg.js — run in Figma with 20 nodes selected.
 * Names: {persona-slug}-bg e.g. 01-young-confidence-seeker-bg …
 * Order from scripts/persona-card-slugs.json. Sort: top-to-bottom, then left-to-right (rows ~12px tolerance).
 *
 *   node scripts/build-rename-persona-bg-eval.mjs
 *   node src/index.js eval --file scripts/.generated-rename-persona-bg.js
 */
...
const OUT = join(__dirname, '.generated-rename-persona-bg.js');

const slugs = JSON.parse(readFileSync(MANIFEST, 'utf8'));
const names = slugs.map((s) => `${s}-bg`);
