...
/**
 * Mahoraga — Figma CLI Bridge
 *
 * Named after the mythical shikigami that adapts to any situation.
 * This bridge adapts CLI commands to work across any environment.
 *
 * Supports two modes:
 ...
 *
 * Security features:
 * - Session token authentication (X-Mahoraga-Token header)
...
async function getFigmaClient() {
  try {
    ...
    if (!FigmaClient || modTime > lastModTime) {
      ...
      lastModTime = modTime;
      if (wasReload) console.log('[mahoraga] Hot-reloaded figma-client.js');
    }
  } catch (e) {
    console.error('[mahoraga] Hot-reload error:', e.message);
...
const PORT = parseInt(process.env.MAHORAGA_PORT) || 3456;
const MODE = process.env.MAHORAGA_MODE || 'auto'; // 'auto', 'cdp', 'plugin'
const IDLE_TIMEOUT_MS = parseInt(process.env.MAHORAGA_IDLE_TIMEOUT) || 10 * 60 * 1000; // Default: 10 minutes

// ============ SECURITY ============

// Load session token (generated by CLI on mahoraga start)
const TOKEN_FILE = join(homedir(), '.figma-ds-cli', '.mahoraga-token');
let SESSION_TOKEN = null;

try {
  SESSION_TOKEN = readFileSync(TOKEN_FILE, 'utf8').trim();
  console.log('[mahoraga] Session token loaded');
} catch {
  console.error('[mahoraga] WARNING: No session token found. Mahoraga will reject all requests.');
}
...
function validateRequest(req) {
  ...
  const token = req.headers['x-mahoraga-token'];
  if (!SESSION_TOKEN) {
...
function resetIdleTimer() {
  ...
  idleTimer = setTimeout(() => {
    ...
    console.log(`[mahoraga] Idle for ${idleSecs}s — auto-shutting down`);
    shutdown();
  }, IDLE_TIMEOUT_MS);
}
...
async function getCdpClient() {
  ...
  if (cdpClient) {
    console.log('[mahoraga] CDP WebSocket closed, reconnecting...');
  ...
  try {
    ...
    console.log('[mahoraga] Connected to Figma via CDP (Yolo Mode)');
  } finally {
      ...
      for (let attempt = 0; attempt <= MAX_RETRIES; attempt++) {
        try {
          ...
          switch (action) {
            ...
            case 'wheel-spin':
              if (isPluginConnected()) {
                pluginWs.send(JSON.stringify({ action: 'wheel-spin' }));
                result = { ok: true };
              } else {
                result = { ok: false, reason: 'plugin not connected' };
              }
              break;
            default:
          ...
          lastError = error;
          console.log(`[mahoraga] Attempt ${attempt + 1} failed: ${error.message}`);

          // For Safe Mode: wait briefly for potential reconnect
          if (attempt < MAX_RETRIES && MODE === 'plugin') {
            console.log('[mahoraga] Safe Mode: waiting for plugin reconnect...');
            // Wait up to 2s for plugin to reconnect
            for (let i = 0; i < 10; i++) {
              await new Promise(r => setTimeout(r, 200));
              if (isPluginConnected()) {
                console.log('[mahoraga] Plugin reconnected, retrying...');
                break;
              }
            }
          }
          ...
          if (attempt < MAX_RETRIES && MODE !== 'plugin' && !isPluginConnected()) {
            console.log('[mahoraga] Reconnecting CDP before retry...');
            try { cdpClient.close(); } catch {}
            cdpClient = null;
            await new Promise(r => setTimeout(r, 200));
          }
...
wss.on('connection', (ws) => {
  console.log('[mahoraga] Plugin connected (Safe Mode)');
  pluginWs = ws;
  ...
  ws.on('message', (data) => {
    ...
    try {
      ...
      if (msg.type === 'hello') {
        console.log(`[mahoraga] Plugin version: ${msg.version}`);
      }
      ...
      console.error('[mahoraga] Plugin message error:', e);
    }
  });

  ws.on('close', () => {
    console.log('[mahoraga] Plugin disconnected');
    pluginWs = null;
  ...
  ws.on('error', (error) => {
    console.error('[mahoraga] Plugin WebSocket error:', error.message);
  });
});

httpServer.listen(PORT, '127.0.0.1', () => {
  console.log(`[mahoraga] Mahoraga running on port ${PORT}`);
  console.log(`[mahoraga] Mode: ${MODE === 'auto' ? 'auto (plugin preferred, CDP fallback)' : MODE}`);
  console.log(`[mahoraga] Idle timeout: ${IDLE_TIMEOUT_MS / 1000}s`);
  console.log(`[mahoraga] Auth: token required`);
});
...
function shutdown() {
  console.log('[mahoraga] Shutting down...');
  if (idleTimer) clearTimeout(idleTimer);
...
if (MODE !== 'plugin') {
  getCdpClient().catch(err => {
    console.log('[mahoraga] CDP not available, waiting for plugin connection...');
  });
}
