(async () => {
  // persona tag1: node src/index.js eval --file scripts/update-persona-tag1-labels.js
  ...
  if (!affirm) {
    return { error: 'Collection Affirmations not found' };
  }
  ...
  if (!lightMode || !darkMode) {
    return { error: 'Need light and dark modes' };
  }
  ...
  for (let i = 0; i < SLUGS.length; i++) {
    ...
    const varName = 'persona-card/' + slug + '/tag1';
